...
export async function POST(request: Request) {
  try {
    ...
    if (currentUser === null) return new Response(null, { status: 401 });

    const json = await request.json();
    const num_of_sessions = json.length;

    if (num_of_sessions === 0) return new Response(null, { status: 400 });
    ...
    // await db.update(gameSessions).set(data).where({ id });
